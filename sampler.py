...
class data_sampler_CFRL(object):
    ...
    def tokenize(self, sample):
        ...
        tokenized_sample['index'] = sample['index']
        if self.config.pattern == 'hardprompt':
            input = self._tokenize_hardprompt_llm2vec(sample)
        # elif self.config.pattern == 'softprompt':
        #     ids, mask = self._tokenize_softprompt(sample)   
        # elif self.config.pattern == 'hybridprompt':
        #     ids, mask = self._tokenize_hybridprompt(sample)                     
    ...
    def _tokenize_hardprompt_llm2vec(sample):
        ...
        # prompt = raw_tokens + ['.'] + ['The', 'relation', 'between', 'e1', 'and', 'e2', 'is', ':'] 
        prompt = raw_tokens + ['.'] + ['The', 'relation', 'between','\"'] + h + ['\"','and','\"'] + t + ['\"','is'] + [':'] 
        ...
        # # Create mask
        # mask = np.zeros(self.max_length, dtype=np.int32)
        # end_index = np.argwhere(np.array(ids) == self.tokenizer.sep_token_id)[0][0]
        # mask[:end_index + 1] = 1 
